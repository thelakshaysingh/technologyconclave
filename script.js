...
// ----- Mobile menu (runs after components.js injects nav) -----
...
// Try immediately; if nav not yet injected, wait for DOMContentLoaded
...
(function () {
  ...
  // Initialize all file drops in the form
  const drops = form.querySelectorAll('.file-drop');
  drops.forEach(drop => {
    const forId = drop.getAttribute('for');
    if (!forId) return;
    const fileInput = form.querySelector('#' + forId);
    if (!fileInput) return;
    const title = drop.querySelector('.fd-title');
    const hint = drop.querySelector('.fd-hint');
  ...
  // Only bind the mock submit handler if data-ajax-submit is not "true"
  ...
  const title = drop ? drop.querySelector('.fd-title') : null;
  const hint = drop ? drop.querySelector('.fd-hint') : null;
  const success = document.getElementById('formSuccess');

  form.addEventListener('submit', async e => {
    e.preventDefault();
    if (fileInput && !fileInput.files.length) {
      if (drop) drop.style.borderColor = 'var(--pink)';
      if (title) title.textContent = 'A PDF file is required';
      if (hint) hint.textContent = 'Please attach a PDF before submitting';
      return;
    }
...
// ----- Nav dropdown focus fix -----
(function () {
  // Prevent dropdowns from staying open when clicking links that open in new tabs
  document.addEventListener('click', function (e) {
    const link = e.target.closest('.nav-dropdown a');
    if (link) {
